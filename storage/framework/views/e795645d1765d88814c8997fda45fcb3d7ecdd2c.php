<div class="footer">
		<div class="f-bg-gyl">
				<div class="col-md-5 gylayouts_footer_grid">
					<h3>Recent <span>Works</span></h3>
					 <ul class="con_inner_text midimg">
						<li><a href="#"><img src="<?php echo e(asset('frontend/images/banner22.jpg')); ?>" alt="" class="img-responsive" /></a></li>
					    <li><a href="#"><img src="<?php echo e(asset('frontend/images/banner33.jpg')); ?>" alt="" class="img-responsive" /></a></li>
						<li><a href="#"><img src="<?php echo e(asset('frontend/images/banner44.jpg')); ?>" alt="" class="img-responsive" /></a></li>
						<li><a href="#"><img src="<?php echo e(asset('frontend/images/banner11.jpg')); ?>" alt="" class="img-responsive" /></a></li>
						<li><a href="#"><img src="<?php echo e(asset('frontend/images/banner22.jpg')); ?>" alt="" class="img-responsive" /></a></li>
					    <li><a href="#"><img src="<?php echo e(asset('frontend/images/banner33.jpg')); ?>" alt="" class="img-responsive" /></a></li>
						 <li><a href="#"><img src="<?php echo e(asset('frontend/images/banner11.jpg')); ?>" alt="" class="img-responsive" /></a></li>
						<li><a href="#"><img src="<?php echo e(asset('frontend/images/banner44.jpg')); ?>" alt="" class="img-responsive" /></a></li>
				     </ul>
					
				</div>
				<div class="col-md-2 gylayouts_footer_grid">
					<h3>Popular <span>Links</span> </h3>
					  <ul class="links">
							<li><a href="<?php echo e(url('customer/bbl')); ?>">Home</a></li>
							<li><a href="<?php echo e(url('customer/services')); ?>">Services</a></li>
							<li><a href="<?php echo e(url('customer/pricing')); ?>">Pricing</a></li>
						
						</ul>
				</div>



<div class="col-md-2 gylayouts_footer_grid">
					<h3>Our <span>Links</span> </h3>
					  <ul class="links">
							<li><a href="<?php echo e(url('customer/about-us')); ?>">About</a></li>
							<li><a href="<?php echo e(url('customer/contact-us')); ?>">Contact Us</a></li>
							<li><a href="<?php echo e(url('customer/testimonial')); ?>">Testimonial</a></li>
						</ul>
				</div>



				<div class="col-md-3 gylayouts_footer_grid">
					<h2>Contact <span>Us</span></h2>
					    <ul class="con_inner_text">
							<li><span class="fa fa-map-marker" aria-hidden="true"></span>1234k Avenue, 4th block, <label> New York City.</label></li>
							<li><span class="fa fa-envelope-o" aria-hidden="true"></span> <a href="mailto:info@example.com">info@example.com</a></li>
							<li><span class="fa fa-phone" aria-hidden="true"></span> +1234 567 567</li>
						</ul>

					<ul class="social_agileinfo">
						<li><a href="#" class="gy_facebook"><i class="fa fa-facebook-f"></i></a></li>
						<li><a href="#" class="gy_twitter"><i class="fa fa-twitter"></i></a></li>
						<li><a href="#" class="gy_instagram"><i class="fa fa-instagram"></i></a></li>
						<li><a href="#" class="gy_google"><i class="fa fa-google-plus"></i></a></li>
					</ul>
				</div>


				<div class="clearfix"> </div>
					
			</div>
			<p class="copyright">© 2018 Body By Lekan. All Rights Reserved |Powered by <a href="http://softcliquesoftware.in/" target="_blank">Softclique Software Services LLP</a></p>
			<!--scroll to top button-->
    		<a id="back2Top" title="Back to top" href="#"><i class="fa fa-hand-o-up"></i></a>

	</div>
	<!-- //footer -->