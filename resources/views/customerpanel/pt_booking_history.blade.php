...
@section('content')                              
      ...
      <div class="tab_container">
          <h3 class="d_active tab_drawer_heading" rel="tab2">Tab 1</h3>
          <div id="tab2" class="tab_content">
             <!-- <a href="{{url('customer/my-pt-booking')}}">Bootcamp</a> -->
             <div id="tabs-inside">
    ...
    <a href="{{url('customer/mybooking')}}" class="ui-tabs-anchor btn btn-success">Bootcamp</a>
 &nbsp;&nbsp;
   <li><a href="#tabs-1">Personal Training </a></li>
  </ul>
          ...
          <select align ="right" id="feature" name="option" >
            ...
            <button type="submit" id="booking" class="btn btn-success"><i class="fa fa-search"></i></button>
                ...
                <tbody id="tbody_empty">
                   @if(count($all_pt_booking)>0)
                   @foreach($all_pt_booking as $key=>$eachbooking)

                  <tr >

                       <td> {{date('d F Y', strtotime($eachbooking->plan_date))}}  </td>
                    <td>{{date('h:i A', strtotime($eachbooking->start_time))}} To {{date('h:i A', strtotime($eachbooking->end_time))}}</td>
                  ...
                  $pt_cancel_time=$eachbooking->plan_date.' '.$eachbooking->start_time;
                  $pt_cancel_time = date("Y-m-d H:i:s", strtotime('-24 hours', strtotime($pt_cancel_time)));
                  ?>
                    <td > @if($current_time<$pt_cancel_time)
                      <a style="margin-left: 9px;" href="{{route('pt_booking_cancele_customer',['id'=>$eachbooking->booking_id])}}"  
